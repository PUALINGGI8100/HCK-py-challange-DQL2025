...
# Fungsi Utama (Revisi Akhir: Output sebagai String Terformat)
def normalize_tanggal_transaksi(input_xlsx_path: str, output_xlsx_path: str) -> None:
    ...
    df_txt = pd.read_csv("dates.txt", sep="\t", names=['date'])
    ...
    df_txt["date"] = df_txt["date"].apply(clean_and_parse_date)

    pola_tanggal_bersih = r'^\d{2}-\d{2}-\d{4}$'
    mask_data_kotor = ~df_txt["date"].astype(str).str.match(pola_tanggal_bersih, na=False)
    df_kotor = df_txt[mask_data_kotor]
    # return print(df_kotor)
    
    # 3. Simpan ke Excel. Kolom akan bertipe General/Teks
